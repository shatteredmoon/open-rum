...
#define ASSET_NATIVE_CLASS "rumPortalAsset"
#define ASSET_STORAGE_NAME "portal"
#define ASSET_TYPE_SUFFIX  "_Portal"


// Static initializations
std::unordered_map< rumAssetID, rumPortalAsset* > rumPortalAsset::s_hashAssets;
rumAssetID rumPortalAsset::s_eNextFreeID{ FULL_ASSET_ID( Portal_AssetType, 0 ) };


// final
void rumPortalAsset::ExportCSVFile( std::ofstream& o_rcOutfile ) const
{
...
// static
void rumPortalAsset::ExportCSVFiles( std::ofstream& o_rcOutfile, std::ofstream& o_rcPropertyOutfile )
{
  // Sort the hash
  using SortedHash = std::map< rumAssetID, rumPortalAsset* >;
  const SortedHash hashAssets( s_hashAssets.begin(), s_hashAssets.end() );

  // Export csv values
  for( const auto& iter : hashAssets )
  {
    const rumPortalAsset* pcAsset{ iter.second };
...
// final
void rumPortalAsset::ExportDBTable( std::string& io_strQuery ) const
{
...
// static
void rumPortalAsset::ExportDBTables( ServiceType i_eServiceType )
{
  // Sort the hash
  using SortedHash = std::map< rumAssetID, rumPortalAsset* >;
  const SortedHash hashAssets( s_hashAssets.begin(), s_hashAssets.end() );
...
// static
rumPortalAsset* rumPortalAsset::Fetch( rumAssetID i_eAssetID )
{
...
// static
const std::string rumPortalAsset::GetAssetClassName()
{
...
// static
const std::string rumPortalAsset::GetAssetTypeSuffix()
{
...
// static
const std::string rumPortalAsset::GetNativeClassName()
{
  return SCRIPT_PORTAL_NATIVE_CLASS;
}


// static
const std::string rumPortalAsset::GetPropertyTableCreateQuery()
{
...
// static
const std::string rumPortalAsset::GetPropertyTableSelectQuery()
{
...
// static
std::string_view rumPortalAsset::GetStorageName()
{
...
// static
const std::string rumPortalAsset::GetTableCreateQuery()
{
...
// static
const std::string rumPortalAsset::GetTableSelectQuery()
{
...
// override
const std::string_view rumPortalAsset::GetTypeName() const
{
  return ASSET_STORAGE_NAME;
}
...
// override
const std::string_view rumPortalAsset::GetTypeSuffix() const
{
  return ASSET_TYPE_SUFFIX;
}
...
// static
int32_t rumPortalAsset::Init( const std::string& i_strPath )
{
  rumAssert( s_hashAssets.empty() );

  // Build the asset hash from any available resource
  LoadAssets<rumPortalAsset>( i_strPath );

  return RESULT_SUCCESS;
}


void rumPortalAsset::OnCreated( const std::vector<std::string>& i_rvFields )
{
...
// static
void rumPortalAsset::ScriptBind()
{
  rumScript::GetOrCreateClassRegistry( GetClassRegistryID() );

  HSQUIRRELVM pcVM{ Sqrat::DefaultVM::Get() };

  Sqrat::RootTable( pcVM ).Func( "rumGetPortalAsset", Fetch );

  Sqrat::DerivedClass<rumPortalAsset, rumPawnAsset> cPortalAsset( pcVM, ASSET_NATIVE_CLASS );
  Sqrat::RootTable( pcVM ).Bind( ASSET_NATIVE_CLASS, cPortalAsset );
