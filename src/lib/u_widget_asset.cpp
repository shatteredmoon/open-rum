...
#define ASSET_NATIVE_CLASS "rumWidgetAsset"
#define ASSET_STORAGE_NAME "widget"
#define ASSET_TYPE_SUFFIX  "_Widget"


// Static initializations
std::unordered_map< rumAssetID, rumWidgetAsset* > rumWidgetAsset::s_hashAssets;
rumAssetID rumWidgetAsset::s_eNextFreeID{ FULL_ASSET_ID( Widget_AssetType, 0 ) };


// final
void rumWidgetAsset::ExportCSVFile( std::ofstream& o_rcOutfile ) const
{
...
// static
void rumWidgetAsset::ExportCSVFiles( std::ofstream& o_rcOutfile, std::ofstream& o_rcPropertyOutfile )
{
  // Sort the hash
  using SortedHash = std::map< rumAssetID, rumWidgetAsset* >;
  const SortedHash hashAssets( s_hashAssets.begin(), s_hashAssets.end() );

  // Export csv values
  for( const auto& iter : hashAssets )
  {
    const rumWidgetAsset* pcAsset{ iter.second };
...
// final
void rumWidgetAsset::ExportDBTable( std::string& io_strQuery ) const
{
  io_strQuery = "INSERT INTO " ASSET_STORAGE_NAME " (type_id,name,baseclass,graphic_type_id_fk,move_flags,blocks_los,collision_flags,light_range,render_priority,service_type) VALUES (";
...
// static
void rumWidgetAsset::ExportDBTables( ServiceType i_eServiceType )
{
  // Sort the hash
  using SortedHash = std::map< rumAssetID, rumWidgetAsset* >;
  const SortedHash hashAssets( s_hashAssets.begin(), s_hashAssets.end() );
...
// static
rumWidgetAsset* rumWidgetAsset::Fetch( rumAssetID i_eAssetID )
{
...
// static
const std::string rumWidgetAsset::GetAssetClassName()
{
...
// static
const std::string rumWidgetAsset::GetAssetTypeSuffix()
{
...
// static
const std::string rumWidgetAsset::GetNativeClassName()
{
  return SCRIPT_WIDGET_NATIVE_CLASS;
}


// static
const std::string rumWidgetAsset::GetPropertyTableCreateQuery()
{
...
// static
const std::string rumWidgetAsset::GetPropertyTableSelectQuery()
{
...
// static
std::string_view rumWidgetAsset::GetStorageName()
{
...
// static
const std::string rumWidgetAsset::GetTableCreateQuery()
{
...
// static
const std::string rumWidgetAsset::GetTableSelectQuery()
{
...
// override
const std::string_view rumWidgetAsset::GetTypeName() const
{
  return ASSET_STORAGE_NAME;
}
...
// override
const std::string_view rumWidgetAsset::GetTypeSuffix() const
{
  return ASSET_TYPE_SUFFIX;
}
...
// static
int32_t rumWidgetAsset::Init( const std::string& i_strPath )
{
  rumAssert( s_hashAssets.empty() );

  // Build the asset hash from any available resource
  LoadAssets<rumWidgetAsset>( i_strPath );

  return RESULT_SUCCESS;
}


void rumWidgetAsset::OnCreated( const std::vector<std::string>& i_rvFields )
{
...
// static
void rumWidgetAsset::ScriptBind()
{
  rumScript::GetOrCreateClassRegistry( GetClassRegistryID() );

  HSQUIRRELVM pcVM{ Sqrat::DefaultVM::Get() };

  Sqrat::RootTable( pcVM ).Func( "rumGetWidgetAsset", Fetch );

  Sqrat::DerivedClass<rumWidgetAsset, rumPawnAsset> cWidgetAsset( pcVM, ASSET_NATIVE_CLASS );
  Sqrat::RootTable( pcVM ).Bind( ASSET_NATIVE_CLASS, cWidgetAsset );
